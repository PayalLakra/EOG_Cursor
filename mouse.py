...
COOLDOWN_SAMPLES = 15      
SAMPLE_RATE = 250          
MOUSE_MOVE_DISTANCE = 30   # Distance in pixels to move the mouse
...
movement_sequence = deque(maxlen=100)  # Large enough buffer
...
def move_mouse(direction):
    """Move mouse left or right based on direction"""
    try:
        if direction == "RIGHT":
            pyautogui.move(MOUSE_MOVE_DISTANCE, 0, duration=0.1)
        elif direction == "LEFT":
            pyautogui.move(-MOUSE_MOVE_DISTANCE, 0, duration=0.1)
...
def check_movement_completion():
    ...
    # Check for RIGHT completion pattern: RIGHT NEUTRAL LEFT NEUTRAL
    for i in range(len(sequence)-3):
        if (sequence[i] == "RIGHT" and 
            sequence[i+1] == "NEUTRAL" and 
            sequence[i+2] == "LEFT" and 
            sequence[i+3] == "NEUTRAL"):
            print("RIGHT SUCCESS")
            move_mouse("RIGHT")  # Move mouse right
    ...
    # Check for LEFT completion pattern: LEFT NEUTRAL RIGHT NEUTRAL
    for i in range(len(sequence)-3):
        if (sequence[i] == "LEFT" and 
            sequence[i+1] == "NEUTRAL" and 
            sequence[i+2] == "RIGHT" and 
            sequence[i+3] == "NEUTRAL"):
            print("LEFT SUCCESS")
            move_mouse("LEFT")  # Move mouse left
            # Remove everything up to and including this pattern
            for _ in range(i+4):
                if movement_sequence:
                    movement_sequence.popleft()
...
# Safety feature - moving mouse to top-left will stop the program
