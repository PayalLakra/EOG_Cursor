...
DEVIATION_SIGMA = 10        
MIN_MOVEMENT_SAMPLES = 10   # Increased for more reliable detection
COOLDOWN_SAMPLES = 20       # Increased cooldown
SAMPLE_RATE = 250          
...
cooldown_counter = 0
last_movement = None
last_detected_state = "NEUTRAL"

# Movement sequence tracking
movement_sequence = deque(maxlen=4)
...
def check_movement_completion():
    ...
    # Need at least 4 elements to check any pattern
    if len(sequence) < 4:
        return False
    
    # Check patterns from newest to oldest
    for i in range(len(sequence)-3, -1, -1):
        # Check for UP completion pattern: UP NEUTRAL DOWN NEUTRAL
        if (sequence[i] == "UP" and 
            i+1 < len(sequence) and sequence[i+1] == "NEUTRAL" and 
            i+2 < len(sequence) and sequence[i+2] == "DOWN" and 
            i+3 < len(sequence) and sequence[i+3] == "NEUTRAL"):
            print("UP SUCCESS")
            # Clear all older movements
            movement_sequence = deque(maxlen=20)
            return True
        
        # Check for DOWN completion pattern: DOWN NEUTRAL UP NEUTRAL
        if (sequence[i] == "DOWN" and 
            i+1 < len(sequence) and sequence[i+1] == "NEUTRAL" and 
            i+2 < len(sequence) and sequence[i+2] == "UP" and 
            i+3 < len(sequence) and sequence[i+3] == "NEUTRAL"):
            print("DOWN SUCCESS")
            # Clear all older movements
            movement_sequence = deque(maxlen=20)
...
try:
    while True:
        ...
        last_detected_state = detected_state
...
# # Movement sequence tracking - only keep last 4 states
# movement_sequence = deque(maxlen=4)
# last_state_change_time = time.time()
# STATE_TIMEOUT = 0.25  # seconds before clearing incomplete sequence
...
# def check_movement_completion():
#     global movement_sequence, last_state_change_time
    
#     # Check if sequence is too old
#     if time.time() - last_state_change_time > STATE_TIMEOUT and len(movement_sequence) < 4:
#         print("eye blink")
#         movement_sequence.clear()
#         return False
    
#     # Need exactly 4 elements to check pattern
#     if len(movement_sequence) != 4:
#         return False
    
#     # Get the sequence as tuple for easy comparison
#     seq = tuple(movement_sequence)
    
#     # Check for UP completion pattern: UP NEUTRAL DOWN NEUTRAL
#     if seq == ("UP", "NEUTRAL", "DOWN", "NEUTRAL"):
#         print("UP SUCCESS")
#         movement_sequence.clear()
#         return True
    
#     # Check for DOWN completion pattern: DOWN NEUTRAL UP NEUTRAL
#     if seq == ("DOWN", "NEUTRAL", "UP", "NEUTRAL"):
#         print("DOWN SUCCESS")
#         movement_sequence.clear()
#         return True
    
#     # If we have 4 states but no match, clear the sequence
#     movement_sequence.clear()
#     return False
